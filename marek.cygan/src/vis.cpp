...
string train_heat_path = "heatmaps/";
...
void load_csv(string train_path) {
  const int T = 1000000;
  char txt[T];
  FILE *f = fopen((train_path+"truth-training.csv").c_str(), "r");
  fgets(txt, T-1, f);
  int lines = 0;
...
void calc_nms(string test_path, string nms_file, double threshold, double conf_threshold, int pieces) {
  DB(conf_threshold);
...
//    vector<pair<double,RectDesc>> rois;
  ...
  FOR(i,1,TRAIN_IMAGES) REP(test,2) if (touched_nms[i][test]) {
    ...
    if (1 || VISUALIZE || test) {
      img = CImg<unsigned char>((test_path+"/3band/3band_AOI_2_RIO_img"+i2s(i)+".tif").c_str());
      testw = img.width();
      testh = img.height();
    }
...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    if (string(argv[i]) == "--test-path") {
      ++i;
      test_path = argv[i];
    } else if (string(argv[i]) == "--train-path") {
      ++i;
      train_path = argv[i];
    } else if (string(argv[i]) == "--scheme") {
      ++i;
      scheme = atoi(argv[i]);
  ...
  if (nms_file != "") {
    calc_nms(test_path, nms_file, nms_threshold, conf_threshold, pieces);
    return 0;
  }
