...
class PascalVOCSeg(PascalVOCDet):
    ...
    def gt_maskdb(self):
        ...
        if os.path.exists(cache_file):
            ...
            group_index = (num_image / 1000) +1
            fid_maskdb = open(cache_file,'wb')

            # step1: just generate each sub_maskdb.pkl, avoiding keeping a global maskdb array which consume too much memory
            for i in range(group_index):
                cache_filename = os.path.join(self.cache_path, self.name + '_' + str(i+1) +'_gt_maskdb.pkl') 
                print 'generating %s file .... ' % cache_filename
                if not os.path.exists(cache_filename):
                    begin_index = i*1000
                    end_index = min(begin_index + 1000,num_image)
                    gt_maskdb_tmp = []
                    gt_maskdb_tmp = [self._load_sbd_mask_annotations(index, gt_roidbs)
                                 for index in xrange(begin_index,end_index)]
                    with open(cache_filename, 'wb') as fid:
                        cPickle.dump(gt_maskdb_tmp, fid, cPickle.HIGHEST_PROTOCOL)
                    fid.close()
                    print 'wrote gt roidb to {}'.format(cache_filename)
                else:
                    continue

            # step2: load and join each sub_maskdb.pkl file
            gt_maskdb = []
            for i in range(group_index):
                cache_filename = os.path.join(self.cache_path, self.name + '_' + str(i+1) +'_gt_maskdb.pkl') 
                print 'load %s file .... ' % cache_filename
                fid_tmp = open(cache_filename,'rb')
                gt_maskdb = gt_maskdb + cPickle.load(fid_tmp)
                fid_tmp.close()

            cPickle.dump(gt_maskdb, fid_maskdb, cPickle.HIGHEST_PROTOCOL)
    ...
    def _load_sbd_mask_annotations(self, index, gt_roidbs):
        ...
        if index % 100 == 0:
            a,b = commands.getstatusoutput('free -h')
            print '%d memory condition :\n' % index, b
