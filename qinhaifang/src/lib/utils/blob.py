...
def im_list_to_blob_mss(im, im_mss):
    """
    Convert a list of images into a network input.
    Assumes images are already prepared (means subtracted, BGR order, ...).
    """
    mss_mean= cfg.MSS_DATA_MEANS
    cols = im.shape[1]
    rows = im.shape[0]
    blob = np.zeros((1, rows, cols, 3 + cfg.MSS_DATA_CHANNELS),
                    dtype=np.float32)
    blob[0, 0:im.shape[0], 0:im.shape[1], 0:3] = im
    for i in range(8):
        data = im_mss[:,:,i]
        data = 255.0*(data-np.min(data))/(np.max(data)-np.min(data))
        data = cv2.resize(data,(cols,rows)) - mss_mean[0][0][i]
        blob[0, 0:rows, 0:cols, 3 + i] = data
