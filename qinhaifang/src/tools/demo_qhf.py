...
if __name__ == '__main__':
    ...
    src_dir = './data/VOCdevkitSDS'
    des_dir = './data/testRes'
    demo_dir = './data/demo'
    
    im_file = open('./data/VOCdevkitSDS/val.txt','r')
    line = im_file.readline()
    im_names = []
    while line:
	list = line.split(' ')
	for word in list:
	    if word[-1]=='\n':
		im_names.append(word[:-1])
	    else:
		im_names.append(word)
	line = im_file.readline()
    im_file.close()

    for img_name in im_names:
        print '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~'
 	im_name = os.pathy.join(img_name + '.tif')
	print 'Demo for data/demo/{}'.format(im_name)
        gt_image = os.path.join(src_dir, im_name)
	...
	pred_dict_name = os.path.join(des_dir,img_name + '.mat')
	sio.savemat(pred_dict_name,{'pred_dict':pred_dict})
        ...
        superimpose_name = os.path.join(demo_dir, 'final_' + img_name + '.jpg')
