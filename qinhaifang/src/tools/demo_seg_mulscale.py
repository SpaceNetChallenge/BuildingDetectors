...
def parse_args():
    ...
    parser.add_argument('--datalist', dest='data_list',
                        help='path to save test results',
                        default='val.txt', type=str)
...
def prepare_mnc_args(im, net, test_size):
    # Prepare image data blob
    blobs = {'data': None}
    processed_ims = []
    im, im_scale_factors = \
        prep_im_for_blob(im, cfg.PIXEL_MEANS, test_size , cfg.TRAIN.MAX_SIZE)
...
if __name__ == '__main__':
    ...
    im_file = open(os.path.join(args.data_path,args.data_list),'r')
	...
	print 'im size:{}'.format(im.shape)
        ...
        print 'seg_scores{}'.format(seg_scores.shape)
        end = time.time()
        ...
        pred_dict_name = os.path.join(res_path,img_name + '.mat')

        img_width = im.shape[1]
        img_height = im.shape[0]
        inst_img, cls_img = _convert_pred_to_image(img_width, img_height, pred_dict)
        sio.savemat(pred_dict_name,{'pred_dict':pred_dict,'inst_img':inst_img,'cls_img':cls_img})
