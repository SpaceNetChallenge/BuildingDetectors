...
def importgeojson(geojsonfilename, removeNoBuildings=False):
    ...
    for idx, feature in enumerate(layer):
        ...
        if poly:
            polys.append({'ImageId': feature.GetField('ImageId'),
                          'BuildingId': feature.GetField('BuildingId'),
...
def createmaskfrompolygons(polygons):
    """docstring for createmaskfrompolygons"""
    pass
...
def geoPolygonToPixelPolygonWKT(geom, inputRaster, targetSR, geomTransform):
    """docstring for geoPolygonToPixelPolygonWKT"""
...
def convert_wgs84geojson_to_pixgeojson(wgs84geojson, inputraster, image_id=None, pixelgeojson=None):
    """docstring for convert_wgs84geojson_to_pixgeojson"""
    if image_id is None:
        image_id = []
    if pixelgeojson is None:
        pixelgeojson = []
        ...
        for feature in layer:
            ...
            for polygonWKT in polygonWKTList:
                ...
                buildinglist.append({'ImageId': image_id,
                                     ...
                                     'poly': ogr.CreateGeometryFromWkt(polygonWKT[0])})  # changed

    if pixelgeojson:
        #exporttogeojson(pixelToLatLon, buildinglist=buildinglist)
        exporttogeojson(pixelgeojson, buildinglist=buildinglist)    # changed

    return buildinglist


def create_rtreefromdict(buildinglist):
    """docstring for create_rtreefromdict"""
    # create index
...
def create_rtree_from_poly(poly_list):
    """docstring for create_rtree_from_poly"""
    # create index
    index = rtree.index.Index(interleaved=False)
...
def createUTMTransform(polyGeom):
    """docstring for createUTMTransform"""
...
def getRasterExtent(srcImage):
    ...
    raster_extend = RasterExtent()
    raster_extend.geoTrans = geoTrans
    raster_extend.poly = poly
    raster_extend.ulX = ulX
    raster_extend.ulY = ulY
    raster_extend.lrX = lrX
    raster_extend.lrY = lrY
    #return geoTrans, poly, ulX, ulY, lrX, lrY
    return raster_extend


def createPolygonFromCorners(lrX, lrY, ulX, ulY):
    """docstring for createPolygonFromCorners"""
    # Create ring
    ring = ogr.Geometry(ogr.wkbLinearRing)
    ring.AddPoint(lrX, lrY)
    ring.AddPoint(lrX, ulY)
...
def cutChipFromMosaic(rasterFile, shapeFileSrc, outlineSrc, outputDirectory='',
        outputPrefix='clip_', clipSizeMX=100, clipSizeMY=100):
    """docstring for cutChipFromMosaic"""
    ...
    raster_extend = getRasterExtent(srcImage)
    geoTrans = raster_extend.geoTrans
    poly = raster_extend.poly
    ulX = raster_extend.ulX
    ulY = raster_extend.ulY
    lrX = raster_extend.lrX
    lrY = raster_extend.lrY
        ...
        for llY in np.arange(minY, maxY, clipSizeMY):
            ...
            if (polyCut).Intersection(geom):
                ...
                outputFileName = os.path.join(outputDirectory,
                        outputPrefix +
                        rasterFileBase.replace('.tif', "_{}_{}.tif".format(minXCut, minYCut)))
                ## Clip Image
                subprocess.call(["gdalwarp", "-te", "{}".format(minXCut), "{}".format(minYCut),
                                 "{}".format(maxXCut), "{}".format(maxYCut),
                                 rasterFile, outputFileName])
                outGeoJSon = outputFileName.replace('.tif', '.geojson')
