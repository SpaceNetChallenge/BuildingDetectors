...
public class ClassificationTree {
	...
	ClassificationTree(float[][] features, boolean[] classif, int idx) {
		...
		root = new ClassificationNode(classifCount, totRows, impurity(classifCount, totRows), 1, 0, numSel - 1);
		int nodeCnt = 0;
		nodes[nodeCnt++] = root;
	...
	ClassificationTree(float[][] features, float[] values, int idx) {
		long t = System.currentTimeMillis();
		int SPLIT_STEPS = 9;
		int MIN_ROWS_PER_NODE = 16;
		int MAX_LEVEL = 24;
