...
#### Official Building Footprint [Metric](https://github.com/SpaceNetChallenge/utilities#evaluation-metric) Score: 
...
#### Appoach:
Uses a fully convolutional network (FCN), mostly following this [paper](https://goo.gl/FlcsMg). In additin, uses custom post-processing developed to merge and polygonise network output.
...
```shell
create_feats.py --band3 3band_AOI_1_RIO_img1194.resized.tif --band8 \ 8band_AOI_1_RIO_img1194.resized.tif \
--geo Geo_AOI_1_RIO_img1194.geojson \
--in-dir infeats --target-dir targetfeats
```
...
crop_feats.py --infeat target_AOI_1_RIO_img1194.npy --outdir outfeat_croped \ 
--to-reshape
```
...
```shell
create_feats.py --band3 3band_AOI_1_RIO_img1194.resized.tif \ --band8 8band_AOI_1_RIO_img1194.resized.tif \
--in-dir feats4prediction --only-infeats
```
Prediction using obtained model is run as following:
```shell
predict.py --model model_final.bin \
--image feats4prediction/in_AOI_1_RIO_img1194.npy \ --outdir predicted
```
